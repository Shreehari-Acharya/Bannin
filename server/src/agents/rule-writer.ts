...
const restartSecurityTool = tool({
  name: "restartTool",
...
const validateGeneratedRules = tool({
  name: "validateRules",
...
const resolveSkillConfigPath = async (): Promise<string> => {
  const preferred = path.join(skillDir, "bannin.yaml");
  const fallback = path.join(skillDir, "bannin.yaml");
...
const loadSkillContext = async (selectedTool: toolname): Promise<string> => {
  const configFilePath = await resolveSkillConfigPath();
  const filesToLoad = [
...
5) Write the final rules file via writeToFile to the exact path from bannin.yaml.
...
${skillContext}
    `,
    model,
    tools: [readRemoteFile, writeRemoteFile, editRemoteFile, validateGeneratedRules, restartSecurityTool],
  });
