...
const logger = createLogger("queue.threat-analysis");
...
const processEventAnalysis = async (eventId: string): Promise<void> => {
  logger.debug("job start", { eventId });
...
export const enqueueThreatAnalysis = (eventId: string): boolean => {
  if (queuedEventIds.has(eventId) || activeEventIds.has(eventId)) {
    logger.debug("skip duplicate enqueue", { eventId });
    return false;
  }

  queuedEventIds.add(eventId);
  queue.push(eventId);
  logger.debug("enqueued", { eventId, pending: queue.length });
  void drainQueue();
  return true;
};
